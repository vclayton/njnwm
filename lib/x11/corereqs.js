...
module.exports = {
	...
	ChangeWindowAttributes: [
		function(wid, values) {
			// writev(3, [{"\2\0\4\0\21\1\0\0\0\10\0\0\0\0\32\0+\0\1\0", 20}], 1) = 20
			// Op: 2, length: 4, wid, valuemask, valuelist
			var format = 'CxSLL';
			var packetLength = 3 + (values ? Object.keys(values).length : 0);
			var args = [2, packetLength, wid];
			var vals = packValueMask('CreateWindow', values); // Attribute values are the same as for CreateWindow
	...
	// Op: 3, length: 2, Params: wid
	GetWindowAttributes: [
		function(wid, parent, x, y)
		{
			return [ 'CxSL', [3, 2, wid] ];
		}
	],
	...
	ReparentWindow: [
		// 7 - opcode, 4 - length. Params: wid(L), parent(L), x(S), y(S)
//		[ 'CxSLLSS', [7, 4] ]
			function(wid, parent, x, y)
			{
				return [ 'CxSLLSS', [7, 4, wid, parent, x, y] ];
			}
		],
	...
	GetAtomName: [
		[ 'CxSL', [17, 2] ],
		function(buf) {
			console.log("GetAtomName reply buffer: ", buf);
